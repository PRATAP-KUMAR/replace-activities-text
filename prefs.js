...
class PrefsWidget {
  ...
  addPictureUrl() {
    ...
    const setting_label = new Gtk.Label({ label: 'Logo', xalign: 0, hexpand: true });
    ...
    this.resetButton.connect('clicked', () => {
      this.gsettings.set_string('icon-path', '/usr/share/icons/Adwaita/scalable/emotes/face-smile-big-symbolic.svg');
    ...
    this.noPicButton.connect('clicked', () => {
      this.gsettings.set_string('icon-path', '');
      this.setting_entry.set_text(this.gsettings.get_string('icon-path'));
    });
  ...
  adjustLogo() {
    const hbox = new Gtk.Box({ orientation: Gtk.Orientation.HORIZONTAL, margin_top: 5 });
    const logo_label = new Gtk.Label({ label: 'Adjust Icon Size', xalign: 0, hexpand: true });
    ...
    this.resetIconButton.connect('clicked', () => {
      this.gsettings.set_double('icon-size', 1.5);
    ...
    return hbox;
    	...
    	tip() {
    	const hbox = new Gtk.Box({ orientation: Gtk.Orientation.HORIZONTAL, margin_top: 5 });
    const tip_label = new Gtk.Label({ label: 'Note: Use Icon of Square Shape for Setting the Logo', xalign: 0, hexpand: true });
    ...
    return hbox;
    	...
    	rightClick() {
    	const hbox = new Gtk.Box({ orientation: Gtk.Orientation.HORIZONTAL, margin_top: 5 });
    const rightClick_label = new Gtk.Label({ label: "Right Click on 'Activities' Area Opens Extension Prefs", xalign: 0, hexpand: true });
  ...
  addTextUrl() {
    ...
    this.resetTextButton.connect('clicked', () => {
      this.gsettings.set_string('text', 'Default');
  ...
  showFileChooserDialog() {
    const fileChooser = new Gtk.FileChooserDialog({ title: 'Select File' });
    ...
    fileChooser.connect('response', (dialog, response) => {
      if (response == Gtk.ResponseType.ACCEPT) {
        ...
        if (file.length > 0) this.setting_entry.set_text(file);
        fileChooser.destroy();
      }
    });
